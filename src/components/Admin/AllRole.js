  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto">
        <PageHeader title="All Roles" setUser={setUser} />
                ...
                roles.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="p-3 border text-center">
                      <button
                        onClick={async () => {
                          if (!window.confirm(`ยืนยันลบโรล ${r.name}?`)) return;

                          try {
                            await api.delete(`/role/${r.id}`);
                            setRoles((prev) =>
                              prev.filter((user) => user.id !== r.id),
                            );
                          } catch (err) {
                            alert("ลบโรลไม่สำเร็จ");
                            console.error(err);
                          }
                        }}
